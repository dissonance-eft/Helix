Derived From:
- /artifacts/run_manifest.json (dataset_hash: 3037aa8f7f03237d3483cb7e200573ce3ff2a834ffc7e061a7f1bae677eb0be4)
- /artifacts/eigenspace/baseline_beams_v2.json
- /artifacts/obstruction/obstruction_spectrum.json
- /artifacts/periodic_atlas/periodic_atlas.json
- /artifacts/risk/risk_scores.json
- /artifacts/invariance/invariance_suite.json
- /artifacts/counterexamples/synthetic_results.json

# Phase 1a Structural Gaps

### Domains resisting geometric framing
- Quantum Error Correction Codes (surface code)
- Constitutional legal systems under judicial review
- Language grammar drift (neutral cultural evolution)
- Nakamoto-style blockchain consensus
- Adaptive immune response (clonal selection)

### Domains where stability is not basin-based
- Quantum Error Correction Codes (surface code)
- Constitutional legal systems under judicial review
- Nakamoto-style blockchain consensus
- Language grammar drift (neutral cultural evolution)
- Traffic flow shockwaves (Lighthill–Whitham–Richards PDE)
- Predator–prey Lotka–Volterra oscillatory ecosystems

### Domains where persistence is not state-based
- Traffic flow shockwaves (Lighthill–Whitham–Richards PDE)
- Language grammar drift (neutral cultural evolution)
- Predator–prey Lotka–Volterra oscillatory ecosystems
- Adaptive immune response (clonal selection)

### Domains where perturbation and maintenance are indistinguishable
- Constitutional legal systems under judicial review
- Adaptive immune response (clonal selection)
- Language grammar drift (neutral cultural evolution)

### Domains where topological invariants appear
- Quantum Error Correction Codes (surface code)
- Traffic flow shockwaves (Lighthill–Whitham–Richards PDE)
- Magnetically confined plasma (tokamak MHD stability)
- Protein folding energy landscapes

### Domains where stochastic structure dominates deterministic dynamics
- Adaptive immune response (clonal selection)
- Language grammar drift (neutral cultural evolution)
