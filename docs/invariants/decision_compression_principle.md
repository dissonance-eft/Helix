...
**Domain hook:** `domains/math/analysis/dcp.py`  
...
## 11. Relationship to Ontology / Irreversibility (EIP)

The **Epistemic Irreversibility Principle (EIP)** describes knowledge acquisition as an irreversible structural transformation. DCP is related but strictly separate:

| Principle | Level | Claims |
|-----------|-------|--------|
| **DCP** | Event-level | Constrained systems compress possibility spaces through identifiable collapse events |
| **EIP** | Process-level | Knowledge acquisition irreversibly alters the epistemic structure |

DCP is compatible with EIP but does not depend on it. A DCP event may or may not involve epistemic irreversibility depending on the domain.
...
| Domain hook: math | ✅ `domains/math/analysis/dcp.py` |
| Domain hook: games | ⚠️ Probe exists (86% pass rate); no formal harness |
| Domain hook: music | ⚠️ Conceptually mapped; no probe |
| Domain hook: cognition | ⚠️ Domain skeleton exists; no probe implemented |
| Domain hook: language | ❌ Not yet mapped |
