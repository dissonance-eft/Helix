...
| `domain` | string | yes | `music`, `language`, `math`, `system` |
...
| `version` | dict | yes | `{schema_version, ccs_version, pipeline_version}` |
...
| Field | Type | Required | Notes |
|-------|------|----------|-------|
| `ccs_embedding` | dict | yes | 6-axis coordinates [0.0, 1.0] |
...
| `Invariant` | math | Structural rule hypothesis | `shares_structure_with`, `tests` |
...
1. **`invalid_entity`**: Missing required fields (ID, type, CCS, confidence, evidence). **System action:** Halt compilation.
...
- **`ccs_version`**: The model version (currently 1.1.0).
...
- **`embeddings_produced`**: resulting CCS coordinates.
