...
**Version:** 1.0
**Location:** `docs/MANIFEST_AUTHORITY.md`
**Applies to:** All manifests under `MANIFEST.yaml`, `domains/*/manifest.yaml`, `applications/*/manifest.yaml`
...
- Every application in `applications/` must have a `manifest.yaml`
- The root `MANIFEST.yaml` must list all canonical domains and applications
...
1. Create the domain/application directory
2. Write `README.md` and `SPEC.md`
3. Create `manifest.yaml` using `manifest_schema_v1`
4. Register it in `MANIFEST.yaml` under `domains:` or `applications:`
