"""Tests for core.hsl.normalizer"""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent))

import pytest
from core.hsl.normalizer import normalize, normalize_command
from core.hsl.errors import HSLSyntaxError, HSLUnsafeCommandError
...
class TestNormalize:
    ...
    def test_empty_raises(self):
        with pytest.raises((HSLSyntaxError, Exception)):
            normalize("")

    def test_unsafe_raises(self):
        with pytest.raises(HSLUnsafeCommandError):
            normalize("rm -rf /")
