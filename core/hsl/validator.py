...
HSL Validator
=============
Semantic validation of parsed HSLCommand AST nodes.
...
Errors returned:
  HSLValidationError, HSLUnknownTargetError, HSLUnsafeCommandError
...
from core.hsl.errors import (
    HSLValidationError, HSLUnknownTargetError,
)
from core.hsl.ontology import (
    OBJECT_TYPES, VALID_ENGINES, is_atlas_backed, plural_key, is_entity_type,
)
from core.hsl.command_registry import get_spec, VALID_VERBS
...
def validate(cmd: HSLCommand, registry: dict | None = None) -> HSLCommand:
    """
    Validate a HSLCommand AST node.
    ...
    for pat in _BLOCKED:
        ...
        if stripped.isalpha() and len(stripped) <= 5:
            if _re.search(rf"\b{_re.escape(stripped)}\b", canonical):
                from core.hsl.errors import HSLUnsafeCommandError
...
def validate_command(cmd: dict | str) -> dict:
    ...
    try:
        if isinstance(cmd, str):
            ...
            if not canonical:
                return {"valid": False, "error": "HSL_VALIDATION_ERROR: empty command"}
            from core.hsl.parser import parse
            ast = parse(canonical)
        validate(ast)
