...
Usage:
    python -m core.governance.validation.structure_validator
    python -m core.governance.validation.structure_validator --verbose
"""
...
# ---------------------------------------------------------------------------
# 4. Application required docs
# ---------------------------------------------------------------------------

APP_REQUIRED_DOCS = ["manifest.yaml"]


def check_application_docs() -> list[StructureViolation]:
    violations = []
    ...
    for app_dir in sorted(apps_dir.iterdir()):
        if not app_dir.is_dir() or app_dir.name.startswith("_"):
            continue
        for required in APP_REQUIRED_DOCS:
            if not (app_dir / required).exists():
                violations.append(StructureViolation(
                    area=f"applications/{app_dir.name}/",
                    file_path=f"applications/{app_dir.name}/{required}",
                    violation_type="MISSING_APP_REQUIRED_DOC",
                    owning_manifest=f"applications/{app_dir.name}/manifest.yaml",
                    recommended_fix=f"Create: applications/{app_dir.name}/{required}",
                ))
    return violations
...
def validate_structure(verbose: bool = False) -> dict:
    ...
    all_violations += check_application_docs()
