...
Usage:
    python -m core.governance.validation.manifest_validator
    python -m core.governance.validation.manifest_validator --root MANIFEST.yaml
"""
...
SYSTEM_REQUIRED = {
    ...
    "layers", "domains", "applications", "governance",
}
...
def validate_system_manifest(path: Path) -> list[ManifestViolation]:
    ...
    actual_domain_dirs = {d.name for d in (ROOT / "domains").iterdir()
...
def validate_all(root_manifest_path: Path | None = None) -> dict:
    ...
    # Domain manifests
    domain_results = {}
    for domain_dir in sorted((ROOT / "domains").iterdir()):
    ...
    app_results = {}
    for app_dir in sorted((ROOT / "applications").iterdir()):
        if not app_dir.is_dir() or app_dir.name.startswith("_"):
            continue
        mpath = app_dir / "manifest.yaml"
        av = validate_app_manifest(mpath)
        app_results[app_dir.name] = {
            "passed": len(av) == 0,
            "violations": [x.to_dict() for x in av],
        }
        all_violations += av
