...
Usage:
    python -m core.governance.validation.manifest_health
    python -m core.governance.validation.manifest_health --verbose
"""
...
def run_health_check(verbose: bool = False) -> dict:
    ...
    # --- Applications ---
    registered_apps = {a["name"]: a for a in (root_m.get("applications") or [])}
    actual_app_dirs = {
        d.name for d in (ROOT / "applications").iterdir()
        if d.is_dir() and not d.name.startswith("_")
    }

    app_health = {}
    for name, entry in registered_apps.items():
        manifest_ref = entry.get("manifest")
        mpath = ROOT / manifest_ref if manifest_ref else None
        check = _check_manifest(mpath) if mpath else {"exists": False, "parseable": False, "status": "NO_PATH"}
        app_health[name] = {
        ...
        if not check["exists"]:
            issues.append(f"APP [{name}] manifest missing: {manifest_ref}")
            system_ok = False

    unregistered_apps = actual_app_dirs - set(registered_apps.keys())
    for name in sorted(unregistered_apps):
        issues.append(f"APP [{name}] directory exists but is NOT registered in system manifest")
        app_health[name] = {"manifest_path": None, "manifest_exists": False, "status": "UNREGISTERED"}
